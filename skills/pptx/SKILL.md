...
- **Connector arrowheads default to none, and a filled triangle is the wrong default when you add one.** Set the head on the line's `<a:ln>`: `<a:tailEnd type="stealth" w="med" len="med"/>` (add `<a:headEnd .../>` for a double arrow). `stealth` is the swept head; `triangle` is the solid one, which reads heavy at print size and swallows the shaft of a short connector. Keep one kind per deck.
